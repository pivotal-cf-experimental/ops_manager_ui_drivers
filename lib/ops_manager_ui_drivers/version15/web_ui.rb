...
module OpsManagerUiDrivers
  module Version15
    class WebUi
      ...
      def setup_page
        Version15::Setup.new(browser: browser)
      end

      def product_dashboard
        Version15::ProductDashboard.new(browser: browser)
      end

      def state_change_progress
        Version15::StateChangeProgress.new(browser: browser)
      end

      def available_products
        Version15::AvailableProducts.new(browser: browser)
      end

      def ops_manager_director
        Version15::OpsManagerDirector.new(browser: browser)
      end

      def product_logs_for(product_name)
        Version15::ProductLogs.new(browser: browser, product_name: product_name)
      end

      def product_resources_configuration(product_name)
        Version15::ProductResourceConfiguration.new(browser: browser, product_name: product_name)
      end

      def product(product_name)
        Version15::ProductConfiguration.new(browser: browser, product_name: product_name)
      end

      def product_status_for(product_name)
        Version15::ProductStatusHelper.new(browser: browser, product_name: product_name)
      end

      def job_network_mapping_for(product_name)
        Version15::JobNetworkMappingHelper.new(browser: browser, product_name: product_name)
      end

      def product_availability_zones(product_name)
        Version15::ProductAvailabilityZones.new(browser: browser, product: product_name)
      end
