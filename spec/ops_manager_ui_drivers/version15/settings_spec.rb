  ...
  module Version15
    RSpec.describe Settings do
      ...
      describe 'AWS settings' do
        ...
        it 'should have the correct fields' do
          fields = aws_settings.fields
          expect(fields['access_key_id']).to eq('AwsAccessKey')
          expect(fields['secret_access_key']).to eq('AwsSecretKey')
          expect(fields['vpc_id']).to eq('VpcId')
          expect(fields['security_group']).to eq('SecurityGroup')
          expect(fields['key_pair_name']).to eq('KeyPairName')
          expect(fields['ssh_private_key']).to eq('SSHKey')
          expect(fields['region']).to eq('Region')
        end
      end
      ...
      describe 'vSphere settings' do
        ...
        it 'should have the correct fields' do
          fields = vsphere_settings.fields
          expect(fields['vcenter_ip']).to eq('CredsIp')
          expect(fields['vcenter_username']).to eq('Username')
          expect(fields['vcenter_password']).to eq('Password')
          expect(fields['datacenter']).to eq('Datacenter')
          expect(fields['datastores_string']).to eq('Datastore')
          expect(fields['microbosh_vm_folder']).to eq('MicroboshVmFolder')
          expect(fields['microbosh_template_folder']).to eq('MicroboshTemplateFolder')
        end
      end
      ...
      describe 'vCloud settings' do
        ...
        it 'should have the correct fields' do
          fields = vcloud_settings.fields
          expect(fields['vcd_url']).to eq('CredsURL')
          expect(fields['organization']).to eq('CredsOrganization')
          expect(fields['vcd_username']).to eq('Username')
          expect(fields['vcd_password']).to eq('Password')
          expect(fields['datacenter']).to eq('VdcName')
          expect(fields['storage_profile']).to eq('VdcStorageProfile')
          expect(fields['catalog_name']).to eq('VdcCatalogName')
        end
      end
    end
  end
